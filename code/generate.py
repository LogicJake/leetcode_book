...
def generate(tag):
    ...
    for t in tag:
        ...
        fp.write("| [{}]({})| {} |  \n".format(name,name.strip().replace(" ","%20").replace("(","%28").replace(")","%29")+"/list.md",t['questions'].__len__()))
            ...
            for r in res:
                if r['question_id'] == id:
                    ...
                    if r['difficulty'] == 1:
                        ...
                        level = "困难"
                    if r['paid_only'] == True:
                        ft.write("| [{}]({}) :lock: | {} |   \n".format(r['question_title'],r['question_title'].strip().replace(" ","%20").replace("(","%28").replace(")","%29")+"/question.md",level))
                    else:
                        ft.write("| [{}]({}) | {} |   \n".format(r['question_title'],r['question_title'].strip().replace(" ","%20").replace("(","%28").replace(")","%29")+"/question.md",level))
